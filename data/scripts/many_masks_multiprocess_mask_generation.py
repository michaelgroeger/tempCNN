...
if __name__ == "__main__":
    for i in tqdm(range(0, 121), desc="Generating Mask.."):
        path_to_tile = os.path.join(
            "'/Volumes/Volume/QGIS/tiled_image/tiled_image." + str(i) + ".tif"
        )
        # Generate data to worked on
        # path_to_tile = os.path.join(
        # "'/Volumes/Volume/QGIS/tiled_image/tiled_image." + str(1) + ".tif"
        # )
        long_lat_grid = return_long_lat_grid(
            "data/outputs/meatadata.json", path_to_tile
        )
